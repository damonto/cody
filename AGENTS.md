...
- `npm run reporting:transfer` copies request history between D1 and standard databases. It copies only finished requests with `ON CONFLICT DO NOTHING`, lets the target's rollup triggers recount them, and copies settled rollups that retention left without requests as one-time adjustments. Read live D1 with queries, never `d1 export`, which blocks the database.
